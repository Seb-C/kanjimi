...
describe('UserController', async function() {
	it('get (normal case)', async function() {
		...
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/${user.id}`, {
			method: 'GET',
			headers: {
	...
	it('get (authentication errors)', async function() {
		...
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/${user2.id}`, {
			method: 'GET',
			headers: {
				Authorization: `Bearer ${apiKey.key}`,
			},
		...
		const response2 = await fetch(process.env.KANJIMI_API_URL + `/user/${user2.id}`, {
			method: 'GET',
			headers: {
	...
	it('create (normal and duplicate case)', async function() {
		...
		expect(mail).toContain(
			`https://localhost/app/verify-email?userId=${dbUser.id}&emailVerificationKey=${encodeURIComponent(<string>dbUser.emailVerificationKey)}`
		);

		// Trying again (it should fail)
		const response2 = await fetch(process.env.KANJIMI_API_URL + '/user', {
			method: 'POST',
			body: JSON.stringify({
				email: 'unittest@example.com',
				password: '123456',
	...
	it('update (not found error)', async function() {
		...
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/00000000-0000-0000-0000-000000000000`, {
			method: 'PATCH',
			headers: {
				Authorization: `Bearer ${apiKey.key}`,
			},
	...
	it('update (not found + not a uuid)', async function() {
		...
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/wrong_id`, {
			method: 'PATCH',
			headers: {
				Authorization: `Bearer ${apiKey.key}`,
			},
	...
	it('verifyEmail (normal case)', async function() {
		...
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/${user.id}/verify-email`, {
			method: 'PATCH',
			body: JSON.stringify({
				emailVerificationKey: 'qwerty',
			}),
	...
	it('verifyEmail (not found error)', async function() {
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/00000000-0000-0000-0000-000000000000/verify-email`, {
			method: 'PATCH',
			body: JSON.stringify({
	...
	it('verifyEmail (not found + not a uuid)', async function() {
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/wrong_id/verify-email`, {
			method: 'PATCH',
			body: JSON.stringify({
				emailVerificationKey: '123456',
			}),
	...
	it('verifyEmail (empty id)', async function() {
		const response = await fetch(process.env.KANJIMI_API_URL + `/user//verify-email`, {
			method: 'PATCH',
			body: JSON.stringify({
				emailVerificationKey: '123456',
			}),
	...
	it('requestResetPassword (normal case)', async function() {
		...
		expect(mail).toContain('To: unittest@example.com');
		expect(mail).toContain(
			`https://localhost/app/reset-password?userId=${user.id}&passwordResetKey=${encodeURIComponent(<string>user.passwordResetKey)}`
		);
	});
	...
	it('resetPassword (normal case)', async function() {
		...
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/${user.id}/reset-password`, {
			method: 'PATCH',
			body: JSON.stringify({
				password: 'qwerty',
				passwordResetKey: 'test_key',
	...
	it('resetPassword (not found error)', async function() {
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/00000000-0000-0000-0000-000000000000/reset-password`, {
			method: 'PATCH',
			body: JSON.stringify({
				password: 'qwerty',
				passwordResetKey: 'test_key',
	...
	it('resetPassword (not found + not a uuid)', async function() {
		const response = await fetch(process.env.KANJIMI_API_URL + `/user/wrong_id/reset-password`, {
			method: 'PATCH',
			body: JSON.stringify({
				password: 'qwerty',
				passwordResetKey: 'test_key',
	...
	it('resetPassword (empty id)', async function() {
		const response = await fetch(process.env.KANJIMI_API_URL + `/user//reset-password`, {
			method: 'PATCH',
			body: JSON.stringify({
				password: 'qwerty',
				passwordResetKey: 'test_key',
